# 파이썬에서 기본 입출력 함수는 input()/output() 가 있습니다.
# 사용자가 터미널에 대해 입/출력이 아닌 파일에 대한 입출력 하기 위해서는 
# 파일 입/출력에 대한 별도의 함수를 사용합니다
...
# readline() : 한줄 씩 내용을 읽음, EOF(End of File)에 도달하면 None 반환
# readlines() : 해당 파일의 모든 라인을 순서 대로 읽어 들임
...
# with 키워드 : 프로그램이 길어지면 open()과 close()사이에 많은 코드가 들어가 
# close를 호출하지 않는 문제가 발생 할 수 있음

# with open("score.txt","r",encoding="utf8") as score_file :
#     print(score_file.read())

# from datetime import datetime
# with open("password.txt","w") as fd : #w면 게속 파일 위에 새로 쓴다 a로 쓰면 추가된다
...
#         password = my_str[:3] + str(len(my_str)) + str(my_str.count("o")) + date +"!" +"lts"
#         fd.write(password+"\n")

# pickle : 파이썬 객체를 직렬화(serialize) 하고 역직렬화(deserialize)하기 위한 모듈
# 객체를 바이너리 형태로 저장 및 복원하기 위해서 사용
import pickle
data={"name" : "이태석", "age" : 22, "city" : "서울시"}
